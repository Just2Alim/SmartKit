...
class B2BLocationsScreen extends StatelessWidget {
  ...
  void _showLocationDialog(BuildContext context, {B2BLocationModel? location}) {
    ...
    final capacityController = TextEditingController(text: location?.capacity.toString() ?? '1000');
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: const [
                    DropdownMenuItem(value: 'Warehouse', child: Text('Склад')),
                    DropdownMenuItem(value: 'Pharmacy', child: Text('Аптека')),
                    DropdownMenuItem(value: 'Storage', child: Text('Хранилище')),
                  ],
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: const [
                    ...
                    DropdownMenuItem(value: 'Full', child: Text('Заполнен')),
                    DropdownMenuItem(value: 'Maintenance', child: Text('Тех. работы')),
                  ],
            ...
            ElevatedButton(
              onPressed: () async {
                final newLocation = B2BLocationModel(
                  ...
                  capacity: int.tryParse(capacityController.text) ?? 1000,
              ...
              child: Text(location == null ? 'Создать' : 'Сохранить', style: const TextStyle(color: Colors.white)),
            ),
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                      ...
                      sliver: SliverList(
                        delegate: SliverChildBuilderDelegate(
                          (context, index) => _LocationCard(
                            ...
                            onEdit: () => _showLocationDialog(context, location: locations[index]),
                          ),
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('Добавить точку', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<B2BInventoryModel>>(
      ...
      builder: (context, snapshot) {
        ...
        final occupancy = location.capacity > 0 ? (totalStock / location.capacity).clamp(0.0, 1.0) : 0.0;
        final statusColor = occupancy >= 1.0 ? const Color(0xFFEF4444) : (location.status == 'Active' ? const Color(0xFF10B981) : const Color(0xFFF59E0B));
        final statusText = occupancy >= 1.0 ? 'Переполнен' : (location.status == 'Active' ? 'Активен' : 'Заполнен');
            ...
            children: [
              Row(
                ...
                children: [
                  Container(
                    ...
                    child: Icon(
                      location.type == 'Warehouse' ? Icons.warehouse_rounded : Icons.local_pharmacy_rounded,
              ...
              Row(
                ...
                children: [
                  Text(
                    'Загрузка хранилища',
                    style: TextStyle(color: const Color(0xFF1E293B).withOpacity(0.8), fontWeight: FontWeight.w700, fontSize: 13),
                  ),
                  Text(
                    '${(occupancy * 100).toInt()}%',
              ...
              Row(
                children: [
                  _infoTile(Icons.inventory_2_outlined, '$itemCount', 'Позиций'),
                  const SizedBox(width: 32),
                  _infoTile(Icons.shopping_basket_outlined, '$totalStock', 'Всего ед.'),
                  const SizedBox(width: 32),
                  _infoTile(Icons.storage_rounded, '${location.capacity}', 'Емкость'),
                ],
              ),
              const SizedBox(height: 24),
              Row(
                children: [
                  Expanded(
                    child: OutlinedButton(
                      ...
                      child: const Text('Изменить', style: TextStyle(color: Color(0xFF1E293B), fontWeight: FontWeight.w800)),
                    ),
                  ),
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => B2BLocationInventoryScreen(location: location),
                          ),
                        );
                      },
                      style: ElevatedButton.styleFrom(
                      ...
                      child: const Text('Инвентарь', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800)),
                    ),
                  ),
                ],
              ),
    ...
    return Row(
      children: [
        ...
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(value, style: const TextStyle(fontWeight: FontWeight.w900, color: Color(0xFF1E293B), fontSize: 14)),
